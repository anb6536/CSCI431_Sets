...
function find_card( fn_in )
    ...
    for idx = 1:max_num
        ...
        if should_exec
            ...
            % Finding the convex hull around each card and then extracting
            % th x and the y values which is then used to crop the image
            [k, ~]     = convhull(rc);
            conv_xs    = rc(k, 1);
            conv_ys    = rc(k, 2);

            % Finding the min and max column and row values from the convex
            % hull x and ys and using that to crop the card out of the
            % original image
            min_x      = min(conv_xs(:));
            max_x      = max(conv_xs(:));
            min_y      = min(conv_ys(:));
            ...
            % Cropped individual card image in RGB
            solo_card_color = im_og(min_y:max_y, min_x:max_x, :);

            % Calling the functions to figure out the shape, texture and
            % color of the individual card and storing the return values in
            % a variable
            ...
            % Performing some operations on the image to get rid of the
            % edges between the card and the black background to send it to
            % the shape classifier
            gray = rgb2gray(solo_card_color);
            gray = gray - 150;
           
            % Binarize the image and then complementing it
            bin_im = imbinarize(gray);
            bin_im = imcomplement(bin_im);

            % Seperating the image into regions
            ...
            % After finding the card region from the image, turning it all
            % into white so that the edges between the card and the black
            % cloth dont exist anymore
            ...
            % Calling the classify shape function to figure out the shape
            % on the card
...
%             colormap( ax(3), gray );
            ...
            % Printing all the info recieved from the function about the
            % card to the command line
            fprintf("Card Num: %d, Color: %s, Number: %d, Shape: %s, Shading: %s\n", card_num, card_color, shape_count, card_shape, card_texture);
            
            % Storing that info in a cell array which is then used by the
            % find set function to find the possible sets in an image.
    ...
    % Calling the function to find all the sets in the given image using
    % the information retrieved from all the other classification functions
    Find_Sets(all_cards);
